...
class OptimizedDragonNet(nn.Module):
    ...
    def __init__(self, input_dim, shared_widths=(200, 200), split_sizes=(100, 100), hidden_tau=64, dropout=0.1, use_pyro=False, dp_k=10):
        super().__init__()
        ...
        # tau head for treatment effect
        self.tau_head = nn.Sequential(nn.Linear(shared_out_dim + 1, hidden_tau), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hidden_tau, 1))
    ...
    def forward(self, x, denoise=False, denoise_steps=50):
        phi = self.shared(x)
        if self.project is not None:
        ...
        return dict(m=m, tau=tau, g=g, phi_a=a, phi_b=b, correction=noise_correction)

    def denoise_residuals(self, resid_numpy, num_steps=200, lr=1e-2, verbose=False):
        """Fit DP-GMM to residuals and return denoised residuals (posterior predictive mean)."""
